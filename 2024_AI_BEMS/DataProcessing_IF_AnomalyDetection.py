...
def preprocess(df, points=4, freqInterval='15min'):
    ...
    # 3. 시간 지연 피쳐 생성
    for lag in range(1, 2):  # 2h delayed pattern
    ...
    # 5. 윈도우 통계 피처 생성
    window_sizes = [1, 2, 3, 4, 5, 6]  # 1시간, 2시간, 3시간 윈도우 통계
    ...
    window_sizes = [1, 2, 3, 4, 5, 6]  # 1일, 2일 윈도우 통계
    for window in window_sizes:
    ...
    return X_df, nan_counts_df, missing_ratio