...
def load_data_from_db():
    """PostgreSQL에서 최근 데이터 로드 (SQLAlchemy 사용)"""
    try:
...
def save_control_signal(result):
    ...
    try:
        ...
        print(f"[{datetime.now()}] 제어신호 {result['control_signal']} (예측값: {result['prediction']:.3f}) DB 저장 완료")
    except Exception as e:
        print(f"[ERROR] 제어신호 저장 실패: {e}")
        print(f"[INFO] DB 저장 실패 - 제어 신호 정보는 위 로그 참조")
...
if __name__ == "__main__":
    ...
    print(f"[{datetime.now()}] 예측 제어 작업 완료")