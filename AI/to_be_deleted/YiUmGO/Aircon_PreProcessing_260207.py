...
def preprocess(raw_df, points=4, freqInterval='15min', only_cleansing=False, fill_method='zero'):
    ...
    if fill_method == 'zero':
        ...
        df['value'] = df['value'].interpolate(method='time')  # 회귀 모델인 경우, 대안으로 선택 가능, 시간 기반 linear 보간
    elif fill_method == 'linear':
        df['value'] = df['value'].interpolate(method='linear')  # 회귀 모델인 경우, 대안으로 선택 가능, 등간격 가정 linear 보간
...
def generate_df_offON_virtual(df_tCur, df_coolHeat, df_offON):
    # Initialize df_offON_virtual with zeros
    df_offON_virtual = pd.DataFrame(0, index=df_tCur.index, columns=['value'])

    # Calculate first derivatives and percentage change of temperature
    df_tCur_diff = df_tCur.diff().fillna(0)
    df_tCur_diff2 = df_tCur_diff.diff().fillna(0)
    df_tCur_pct = df_tCur.pct_change(2)

    # Iterate through time points
    k_start = None
    for k in range(2, len(df_coolHeat)-2):
        # 냉방/난방 모드가 변경되는 지점을 찾음
        if abs(df_coolHeat.diff().fillna(0).iloc[k]['value']) > 0:
            k_start = None
        # For cooling mode (df_coolHeat == 0)
        if df_coolHeat.iloc[k]['value'] == 0 and df_offON.iloc[k]['value'] < 1.0:
            # Find start point where all derivatives are positive
            if (k_start is None and
                df_tCur_diff.iloc[k]['value'] <= 0.0 and 
                df_tCur_diff.iloc[k+1]['value'] < -0.1 and
                df_tCur_diff2.iloc[k]['value'] < -0.1 and
                df_tCur_pct.iloc[k+1]['value'] < -0.05):
                k_start = k-1
            # Find end point where derivatives become negative
            elif (k_start is not None and
                  df_tCur_diff.iloc[k]['value'] >= 0.0 and
                  df_tCur_diff.iloc[k+1]['value'] > 0.1 and
                  df_tCur_diff.iloc[k+2]['value'] > 0.1 and 
                  df_tCur_diff2.iloc[k]['value'] > 0.1 and 
                  df_tCur_pct.iloc[k+2]['value'] > 0.03):
                df_offON_virtual.iloc[k_start:k] = 1
                k_start = None

        # For heating mode (df_coolHeat == 1)        
        elif df_coolHeat.iloc[k]['value'] == 1 and df_offON.iloc[k]['value'] < 1.0:
            # Find start point where all derivatives are negative
            if (k_start is None and
                df_tCur_diff.iloc[k]['value'] >= 0.0 and
                df_tCur_diff.iloc[k+1]['value'] > 0.1 and
                df_tCur_diff2.iloc[k]['value'] > 0.1 and
                df_tCur_pct.iloc[k+1]['value'] > 0.08):
                k_start = k-1
            # Find end point where derivatives become positive    
            elif (k_start is not None and
                  df_tCur_diff.iloc[k]['value'] <= 0.0 and
                  df_tCur_diff.iloc[k+1]['value'] < -0.1 and
                  df_tCur_diff.iloc[k+2]['value'] < -0.1 and
                  df_tCur_diff2.iloc[k]['value']  < -0.1 and
                  df_tCur_pct.iloc[k+1]['value'] < -0.03):
                df_offON_virtual.iloc[k_start:k] = 1
                k_start = None
        else:
            k_start = None

    return df_offON_virtual


def find_zero_sequence_intervals(df, min_zero_streak):
    # 시작 인덱스와 종료 인덱스를 담을 리스트 초기화
    zero_intervals = []
    # 값이 0인 부분을 True로 마스킹
    is_zero = df['value'] == 0
    # 연속된 True의 그룹을 찾기 위해 shift를 이용해 연속 구간 확인
    zero_streak = (is_zero != is_zero.shift()).cumsum()

    # 그룹별로 크기를 계산하고, 조건에 맞는 인덱스 추출
    for streak_id, group in df[is_zero].groupby(zero_streak):
        if len(group) >= min_zero_streak:
            start_index = group.index[0]  # 시작 인덱스
            end_index = group.index[-1]   # 종료 인덱스
            zero_intervals.append((start_index, end_index))

    return zero_intervals


def generate_tEnv_from_df_tCur(df_tCur, df_offON_adj, pre_change_mask=10, post_change_mask=25):
    """
    Generate environmental temperature (tEnv) from current temperature (tCur) and ON/OFF status
    
    Args:
        df_tCur (pd.DataFrame): Current temperature data with 'value' column
        df_offON_adj (pd.DataFrame): Adjusted ON/OFF status with 'value' column
        pre_change_mask (int): Number of points to mask before ON status change (default: 10)
        post_change_mask (int): Number of points to mask after OFF status change (default: 25)
    
    Returns:
        pd.DataFrame: Environmental temperature data with 'value' column
    """
    # Initialize mask
    mask = pd.Series(0, index=df_offON_adj.index)

    # Find status change points
    change_to_1 = df_offON_adj[(df_offON_adj['value'] == 1) & (df_offON_adj['value'].shift(1) == 0)].index
    change_to_0 = df_offON_adj[(df_offON_adj['value'] == 0) & (df_offON_adj['value'].shift(1) == 1)].index

    # Set mask for ON status changes
    for idx in change_to_1:
        loc = df_offON_adj.index.get_loc(idx)
        start_loc = max(loc - pre_change_mask, 0)
        end_loc = loc + 1
        mask.iloc[start_loc:end_loc] = 1

    # Set mask for OFF status changes
    for idx in change_to_0:
        loc = df_offON_adj.index.get_loc(idx)
        start_loc = loc
        end_loc = min(loc + post_change_mask + 1, len(df_offON_adj))
        mask.iloc[start_loc:end_loc] = 1

    # Convert mask to DataFrame and combine with df_offON_adj
    df_offON_mask = mask.to_frame(name='value')
    df_offON_mask['value'] = df_offON_mask['value'] | df_offON_adj['value']

    # Generate tEnv by masking tCur
    df_tEnv = df_tCur.copy()
    df_tEnv[df_offON_mask['value'] != 0] = np.nan
    df_tEnv.iloc[0] = df_tCur.iloc[0]  # df_tEnv의 첫 째 열이 np.nan이 되지 않도록 함
    df_tEnv, _, _, _ = preprocess(df_tEnv, points=4, freqInterval='15min', only_cleansing=True, fill_method='time')

    # Apply low-pass Butterworth filter
    tEnv_values = df_tEnv['value'].values
    order = 3
    cutoff_freq = 0.003
    nyquist = 0.5
    normalized_cutoff_freq = cutoff_freq / nyquist
    b, a = signal.butter(order, normalized_cutoff_freq, btype='low', analog=False)
    tEnv_filtered = signal.filtfilt(b, a, tEnv_values)
    df_tEnv['value'] = tEnv_filtered

    return df_tEnv


def fit_exponential(y, mode='decay'):
    """
    Fit an exponential curve to the data and return the time constant (tau)
    """
    t = np.arange(len(y))
    # 모드에 따른 부호 설정
    sign = 1 if mode == 'decay' else -1  # 'decay' or 'growth'

    # 초기 추정값 및 경계 설정
    initial_guess = [6000, 11100, 1.0]  # A, B, tau
    bounds = ([0, 0, 0], [10000, 20000, 10])

    # 모델 함수 정의 (함수 내부에서 직접 정의)
    def model_func(t, A, B, tau):
        return A * np.exp(-t / tau) * sign + B
