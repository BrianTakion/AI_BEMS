...
def preprocess(raw_df, points=4, freqInterval='15min', only_cleansing=False):
    df = raw_df.copy()
    ...
    if only_cleansing:
        ...
        return df, nan_counts_df, missing_ratio
    ...
    for i in range(level + 1):
        if i == 0:
            ...
            features_dict[f'wavelet_app'] = np.interp(np.linspace(0, len(coeffs[i]) - 1, len(df_value1p)), np.arange(len(coeffs[i])), coeffs[i])
        else:
            # Detail coefficients
            features_dict[f'wavelet_det_{i}'] = np.interp(np.linspace(0, len(coeffs[i]) - 1, len(df_value1p)), np.arange(len(coeffs[i])), coeffs[i])
