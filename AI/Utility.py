        ...
        for idx, df in enumerate(dfL):
            ...
            if normalizing:
                ...
                createFig.add_trace(go.Scatter(x=collect_dt, y=val, 
                                               mode='markers', marker=dict(size=4), name=legendL[idx]+f', max={cal_max:.1f}'))
                yrange_min = min(val)*1.1 if min(val) < yrange_min else yrange_min
...
def plot_interpolated_data(df_raw, df_interpol, plotType='simple', title=None, W=None, H=None):
    if plotType == 'simple':
        ...
        # Original Data
        cal_max_raw = df_raw['colec_val'].max()
        normalized_val_raw = df_raw['colec_val'] / cal_max_raw if abs(cal_max_raw) > 0.01 else df_raw['colec_val'] / (cal_max_raw + 0.01)
        createFig.add_trace(
        ...
        # Interpolated Data
        cal_max_interpol = df_interpol['colec_val'].max()
        normalized_val_interpol = df_interpol['colec_val'] / cal_max_interpol if abs(cal_max_interpol) > 0.01 else df_interpol['colec_val'] / (cal_max_interpol + 0.01)
        createFig.add_trace(
